        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1" style="max-height: 500px; overflow-y: auto;">
                ...
                <ul class="nav navbar-nav navbar-right">
                    ...
                    <?php 
                    $qry="SELECT l.*,s.s_name, DATEDIFF(l.a_to,CURDATE()) as day_diff   FROM apply_leave_list as l 
                    INNER JOIN student as s ON s.id=l.s_id 
                    WHERE l.a_status='PENDING' AND DATE(l.a_to) <= DATE_ADD(CURDATE(), INTERVAL 2 DAY) ORDER BY id DESC LIMIT 5";
                    $res=mysqli_query($conn,$qry);
                    $num=mysqli_num_rows($res);
                    $span="";
                    if($num >0){
                       $span='<span class="badge" style="background-color: red;">'.$num.'</span>';
                    }
                    ?>
                    <li class="dropdown">
                  <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">
                  <spam class="glyphicon glyphicon-bell">  <?=$span?></span>
                  
                  </a>
                  <ul class="dropdown-menu">
                    <!-- <li><a href="#">New student registered</a></li> -->
                    <?php
                    ...
                    ?>
                      <li>
                        <a href="admin_leave_form.php?id=<?=$row["id"]?>">
                          <?=$row["s_name"]?> 
                          <span style="font-size: 10px; background-color: <?=$background?>; color: white; border-radius: 12px; padding: 2px 6px; display: inline-block;">
                            <?php 
                            if($row["day_diff"] > 0){
                              echo $row["day_diff"] . " day(s) left";
                            } else {
                              echo "Passed " . abs($row["day_diff"]) . " day(s)";
                            }
                            ?>
                          </span>
                        </a> 
                    ...
                    <?php } ?>
                    <hr>
                    <li><a href="studentapplyleavelist.php">View all student leave</a></li>
