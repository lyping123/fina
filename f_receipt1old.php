	...
	/*if($c_rows > 0){*/
		...
		if($result_save = mysqli_query($conn, $qry_save)){
			$r_id = mysqli_insert_id($conn);
			   ...
			   if($_POST['p_type']=="Tuition Fee" || $_POST['p_type']=="Tuition PTPK" ||  $_POST['p_type']=="Debtor PTPK"){
                   $amount=0;
			 foreach($price as $pri_each){
			     $amount+=$pri_each;
			 }
 $up="update student set outstanding=outstanding-".$amount.",tuition_fee_left=tuition_fee_left-".$amount.",rep_date ='".$_POST['date']."' where id='".$_POST['name']."'";
