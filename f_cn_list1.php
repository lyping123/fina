...
if(isset($_GET['search']) && $_GET['search'] == 'search'){
	$s_name .= " AND f.s_name LIKE '%".mysqli_real_escape_string($conn,$_GET['name'])."%'";
}else{
	$s_name = '';
}
?>
...
<?php 
	...
	$qry_rcp = "SELECT * FROM f_cn AS f
                INNER JOIN student as s on s.id=f.s_id
                INNER JOIN login AS l ON l.id = f.createby
                WHERE f.cn_status = 'ACTIVE'".$s_name." AND f.receipt_type = '1' 
                ORDER BY f.id DESC";
        ...
        <div class="row">
            ...
            <table id="mytable" class="table table-bordred table-striped" style="width:100%">
				...
				?>
                	<tr>
                        ...
                        <div class="dropdown">
                          ...
                          <ul class="dropdown-menu dropdown-menu-right">
                            <li><a href="f_cn_print.php?&id=<?=$row_rcp[0]?>&&newid=<?=$row_rcp[""]?>" target="_blank"><i class="icon-print"></i> Print <!-- (Synergy Central Academy) --></a></li>
                            <!-- <li><a href="f_cn_print_1.php?&id=<?=$row_rcp['id']?>" target="_blank"><i class="icon-print"></i> Print (Pusat Kemahiran Telekomunikasi Mikro)</a></li> -->
                          </ul>
                        </div>
                        </td>
                    <?php if(isset($_SESSION['level']) && $_SESSION['level'] == 'superadmin'){?>
                  ...
                  <ul class="dropdown-menu dropdown-menu-right">
                    <li><a href="f_cn_edit1.php?&id=<?=$row_rcp['id']?>">Edit</a></li>
                    <li><a href="f_cn1.php?action=delete_receipt&id=<?=$row_rcp['id']?>">Delete</a></li>
                  </ul>
                </div></td>
                    <?php }?>
                    </tr>
