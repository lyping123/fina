...
// $qry = "SELECT * FROM f_cn AS r
// 		LEFT JOIN f_b_c AS bc ON bc.r_id = r.id 
// 		WHERE r.id = '".$_GET['id']."'";

$qry = "SELECT *,r.s_name AS old_name, r.createby as newid FROM f_cn AS r    
...
if($row['cn_no'] == ''){
	$rno_qry = "SELECT count(fr.id) AS r_no FROM f_cn AS fr WHERE fr.cn_status = 'ACTIVE' AND fr.receipt_type = '".$row['receipt_type']."' AND fr.id BETWEEN 1 AND ".$row[0];
...
<div style=" width:90%; margin-left:auto; margin-right:auto; padding:10px">
  <table width="100%" border="0" cellspacing="0" cellpadding="4">
    <tr>
        ...
        <td colspan="3" width="100%">
            <div style="display:inline-block; padding:0px 20px">
                ...
                Tel : 04-3984787 &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<br />
                No. 6, 8, 10, Jalan Perai Jaya 1, Bandar Perai Jaya, 13600 Perai, Penang.<br />
                No. 32, 34, 40, 42, 44, 46, 48, Jalan Perai Jaya 4, Bandar Perai Jaya, 13600 Perai, Penang.<br />
                </small></p>
            </div>
        </td>
    </tr>
    <tr>
        <td colspan="4" align="center"><strong><small ><spam style="font-size:23px;padding: 5px;" class="border" > CREDIT NOTE </spam></small></strong></td>
    </tr>
    <tr>
        <td colspan="2">
    <br />
    <br />
        ...
        <td colspan="1" valign="bottom" width="10%" align="right">
            <div style="<!--direction:rtl-->;text-align: left;margin-left: 40px;">
                <strong>No</strong>: <?=$r_no?><br />
                <strong>Date</strong>: <?=date_format(new DateTime($row['cn_date']),'d-m-Y')?><br>
                <strong>Org. Bill No</strong> : <?=$row['org_bill_no']?>
            </div>
        </td>
    </tr>
  </table>
  ...
  <tr bgcolor="#999999">
    ...
    <th colspan="2" align="center" class="border" style="width: 25%">Amount</th>
  </tr>
  <tr bgcolor="#999999">
  ...
  <?php 
  	$rd_qry = "SELECT * FROM f_cn_detail WHERE cn_id = '".$_GET['id']."'";
  ...
  ?>
  <tr>
    <td align="left" class="border" colspan="2" style="padding-left: 10px"><?=nl2br($rd_row['cn_desc'])?></td>
