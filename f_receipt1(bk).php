...
if(isset($_POST['submit']) && $_POST['submit'] == 'save'){
	...
	$c_qry = "SELECT * FROM f_cart WHERE u_id='".$_SESSION['id']."'";
	...
	if($c_rows > 0){
		...
		if(isset($_POST['c_type']) && $_POST['c_type'] == '2' && empty($_POST['p_type'])){
			echo "<script>
			window.location.href = 'f_receipt_form1.php?action=msg_error';
			</script>";
        }else{
            ...
            if($result_save = mysqli_query($conn, $qry_save)){
                $r_id = mysqli_insert_id($conn);
                   ...
                   if($_POST['p_type']=="Tuition Fee" || $_POST['p_type']=="Tuition PTPK" || $_POST['p_type']=="Debtor PTPK" || $_POST['p_type']=="Debtor"){
                 $sum_query="select sum(c_amount)as amount from f_cart where u_id='".$_SESSION['id']."'";
                $sttr_sum=mysqli_query($conn,$sum_query);
                $result_add=mysqli_fetch_array($sttr_sum);
                
                /*$up="update student set outstanding=outstanding-'".$result_add['amount']."',tuition_fee_left=tuition_fee_left-'".$result_add['amount']."',rep_date ='".$_POST['date']."' where id='".$_POST['name']."'";
                $sttr_up=mysqli_query($conn,$up);*/
                 $query_tui="insert into tuitionfee_detail(s_id,r_id,p_month,rep_date)values('".$_POST['name']."','".$r_id."','".$_POST['stage']."','".$_POST['date']."')";
                $sttr_tui=mysqli_query($conn,$query_tui);
            }elseif($_POST['p_type']=="Internal Exam Fee"){

                 $int_update="update internal_fee set pay_status='YES',date_pay='".$_POST['date']."',rep_id='".$r_id."' where s_id='".$_POST['name']."'";
