...
$qry1 = "SELECT *, s.id as newid FROM student_group_list AS sgl
		INNER JOIN student AS s ON s.id = sgl.s_id
		WHERE sgl.g_id = '".$_GET['id']."' AND sgl.status = 'ACTIVE' AND s.s_status != 'DELETE'";
                ...
                <!--<div class="table-responsive">-->
                    ...
                    <table id="example1" class="table table-bordred table-striped" style="width:100%">
                        ...
                        <thead>
                            <th>Student Name</th>
                            <th>Student IC</th>
                            <th>Internship Start date to enddate</th>
                            <th>File Name</th>
                            <th>Payment Type</th>
                            <th>Amount</th>
                            <th>Bank Draf No</th>
                        ...
                        <tbody>
							<?php while($row1 = mysqli_fetch_array($result1)){
                                $qry2="select * from internship where s_name='$row1[newid]'";
                                $sttr2=mysqli_query($conn,$qry2);
                                $fetch=mysqli_fetch_array($sttr2);
                                if($fetch){
                                    $date=$fetch["start_internship"]." to ".$fetch["end_internship"];
                                }else{
                                    $date="";
                                }
                        ...
                        <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> Action <span class="caret"></span>
                            ...
                            <ul class="dropdown-menu  dropdown-menu-right">
                                <li><a href="#" data-title="Add" data-toggle="modal" data-target="#edit<?=$row1[0]?>" type="button" class="edit_button" id="edit_button"> Edit </a></li>
...
<div class="modal fade" id="edit<?=$row1[0]?>" tabindex="-1" role="dialog" aria-labelledby="edit" aria-hidden="true">
