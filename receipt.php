...
if(isset($_POST['submit']) && $_POST['submit'] == 'save'){
	...
	if($c_rows > 0){
		...
		if(isset($_POST['tuition']) && $_POST['tuition'] == 'tuition'){
			...
			$qry_rcp = "SELECT * FROM receipt WHERE tuition_fee = 'YES'";
		...
		if(isset($_POST['hostel']) && $_POST['hostel'] == 'hostel'){
			...
			$qry_rcp = "SELECT * FROM receipt WHERE hostel_fee = 'YES'";
			$result_rcp = mysqli_query($conn, $qry_rcp);
			$c_row = mysqli_num_rows($result_rcp);
			$total_no = $no + $c_row;
			$new_no = 'H'.$total_no;
		...
		if(isset($_POST['internal']) && $_POST['internal'] == 'internal'){
			...
			$qry_rcp = "SELECT * FROM receipt WHERE internal_exam_fee = 'YES'";
			$result_rcp = mysqli_query($conn, $qry_rcp);
			$c_row = mysqli_num_rows($result_rcp);
			$total_no = $no + $c_row;
			$new_no = 'I'.$total_no;
		...
		if(isset($_POST['debtor']) && $_POST['debtor'] == 'debtor'){
			...
			$qry_rcp = "SELECT * FROM receipt WHERE debtor = 'YES'";
			$result_rcp = mysqli_query($conn, $qry_rcp);
			$c_row = mysqli_num_rows($result_rcp);
			$total_no = $no + $c_row;
			$new_no = 'D'.$total_no;
		...
		$qry_save = "INSERT INTO receipt(debtor,internal_exam_fee,r_no, r_date, s_name, s_ic, pay_mtd, tuition_fee, ptpk, r_status, createdate, createby, hostel_fee)VALUES('".$debtor."', '".$internal."', '".$new_no."', '".$_POST['date']."', '".$_POST['name']."', '".$_POST['ic']."', '".$pay_mtd."', '".$tuition."', '".$loan."', 'ACTIVE', '".DATE_TODAY."', '".$_SESSION['id']."', '".$hostel."')";
					...
					while($row = mysqli_fetch_array($result)){
						...
						if($result_rd = mysqli_query($conn, $qry_rd)){
							...
							if($result_del = mysqli_query($conn, $qry_del)){
								...
								echo "<script>
										window.location.href = 'choose.php?action=msg_choose&id=$r_id';
								</script>";
							}else{
								echo "<script>
...
/*								echo "<script>
		...
		echo "<script>
		window.location.href = 'receipt_form.php?action=msg_cart_empty';
		</script>";
	}
}
?>