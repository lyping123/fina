...
<?php 
	...
	$s_qry = "SELECT i_fee FROM student WHERE id = '".$_GET['id']."' AND s_status = 'ACTIVE'";
	$s_result = mysqli_query($conn,$s_qry);
	$s_row = mysqli_fetch_array($s_result);
		
	$qry_rcp = "SELECT * FROM receipt WHERE r_status = 'ACTIVE' 
				AND s_name = '".$_GET['name']."' AND s_ic = '".$_GET['ic']."' 
				AND internal_exam_fee = 'YES' ORDER BY id DESC";
