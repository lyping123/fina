...
$qry = "SELECT *,r.s_name AS old_name FROM f_receipt AS r
...
if($row['r_no'] == ''){
	$rno_qry = "SELECT count(fr.id) AS r_no FROM f_receipt AS fr WHERE fr.r_status = 'ACTIVE' AND fr.receipt_type = '".$row['receipt_type']."' AND fr.cash_bill_option = 'Debtor' AND fr.id BETWEEN 1 AND ".$row[0];
