      ...
      // if everything is ok, try to upload file
        ...
        if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
          ...
          foreach($_POST["student"] as $student){
            $select="SELECT * FROM student_rpk where s_id='$student'";
            $sttr_select=mysqli_query($conn,$select);
            $num_select=mysqli_num_rows($sttr_select);
            if($num_select>0){
              $qry="UPDATE student_rpk SET `img`= '$target_file' WHERE s_id='$student'";
            ...
            $array[]="('$student','$target_file','$_POST[lvl]')";
            $updateStatus=false;
          }
          }
          ...
          if($updateStatus==true){
            echo "<script>
                alert('rpk is updated success');
              </script>";
          }else{
            $qry="INSERT INTO `student_rpk`(`s_id`, `img`,`lvl`) VALUES".$new_sql;
            if(mysqli_query($conn,$qry)){
              echo "<script>
                  alert('add rpk success');
                </script>";
            }else{
              echo "<script>
                  alert('add rpk fail');
...
if(isset($_POST["delete"])){
    $select="SELECT * FROM student_rpk where id='$_POST[delete]'";
    $select_sttr=mysqli_query($conn,$select);
    $row=mysqli_fetch_array($select_sttr);


    $qry="DELETE FROM student_rpk where id='$_POST[delete]'";
...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            <form action="student_rpk.php" method="post" enctype="multipart/form-data">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3>Add RPK to student</h3>
                </div>
                <div class="panel-body" >
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            <label>level</label>
                            <input type="text" name="lvl" class="form-control" placeholder="Enter level" required>
                        </div>
                        <div class="form-group">
                            <label>Student PRK file</label>
...
<?php 
$qry="SELECT s.s_name,sc.img,sc.lvl,course,sc.id FROM student s INNER JOIN student_rpk sc ON s.id=sc.s_id
                  ...
                  <tbody>
                      <?php while($result=mysqli_fetch_array($sttr)){ ?>
                          <tr>
                            ...
                            <td><?=$result["lvl"]?></td>
                            <td><?=$result["course"]?></td>
                            <form action="student_rpk.php" method="post">
...
<script>
    ...
    function checkstudent(id,name){
        let lvl=document.querySelector("input[name='lvl']").value;
        ...
        xhttp.open("GET", `checkstudent.php?rpk=${id}&lvl=${lvl}`, true);
        xhttp.send();
