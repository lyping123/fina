...
if(isset($_GET['search']) && $_GET['search'] == 'search'){
    ...
    if(isset($_GET["s_date"]) && !empty($_GET["s_date"])){
        $searchdate=" AND DATE(f.createdate)>=DATE('$_GET[s_date]') AND DATE(f.createdate)<=DATE('$_GET[e_date]')";
    }
	$qry_rcp = "SELECT 
                    f.id,
                    f.pay_mtd,
                ...
                WHERE
                f.r_status = 'ACTIVE'".$c_type.$searchdate."
                GROUP BY f.id
                ORDER BY f.id DESC
                ";
...
//     var table = $('#table_receipt').DataTable( {
        
//         "ajax": "ajax/load_receipt.php?c_type=1",
//         "columns": [
//             { "data": "r_no" },
//             { "data": "r_date" },
//             { "data": "s_name" },
