...
$row_n=mysqli_fetch_array($sttr);
                        ...
                        if($row['receipt_type'] == 2){
                            ...
                            if($row_rcp['r_no'] == ''){
                                $rno_qry = "SELECT count(fr.id) AS r_no FROM f_receipt AS fr WHERE fr.r_status = 'ACTIVE' AND fr.receipt_type = '".$row['receipt_type']."' AND fr.id BETWEEN 1 AND ".$_GET['id'];
                                $rno_result = mysqli_query($conn, $rno_qry);
                                $rno_row = mysqli_fetch_array($rno_result);
                                $r_no = 10000 + $rno_row['r_no'];
                                $r_no = 'D'.$r_no;
...
<!-- onLoad="window.print()" -->
<button type="button" id="pdf">Click</button>
...
<div style=" width:90%; margin-left:auto; margin-right:auto; padding:10px">
  ...
  <tr bgcolor="#999999">
    <th  align="center" class="border" >Description</th>
    <th align="center" class="border">Amount</th>
  </tr>
  <tr bgcolor="#999999">
  ...
  ?>
  <tr>
    <td align="center" class="border" ><?=$rd_row['rp_desc']?></td>
