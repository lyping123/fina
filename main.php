...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">New Students Not Yet Register
            <!--<small>Secondary Text</small>-->
...
<div class="row">
        	<div class="col-md-12">
                <!--<div class="table-responsive">-->
                    ...
                    <table id="example8" class="table table-bordred table-striped" style="width:100%">
                        ...
                        <tbody>
							<?php 
								$qry1 = "select *,s.s_name as studentname ,n.course as newcourse from none_jpk as n inner join student as s on s.id=n.s_name where n.s_status='ACTIVE' AND n.s_name NOT IN(select sg.s_id from student_group_list as sg where sg.status='ACTIVE')";
...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">Waiting for Register Peningkatan Tahap
            <!--<small>Secondary Text</small>-->
...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">Student Haven't Register Under JPK List
            <!--<small>Secondary Text</small>-->
            <a class="btn btn-primary pull-right" href="print_alert.php?action=jpk" target="_blank"><span class="glyphicon glyphicon-plus"></span> Print </a>
        </h1>
    </div>
                    ...
                    <table id="example4" class="table table-bordred table-striped" style="width:100%">
                        ...
                        <tbody>
							...
							while($row = mysqli_fetch_array($result)){
								...
								if($i>0){
                                    $time=strtotime($row_sttr['createdate']);
                                    $newdate=date("d-m-Y",strtotime($row_sttr['createdate']));	
                            ...
                            <td><?=$row['s_date']?></td>
                            <td><?=$row['e_date']?></td>
                            <td><?php if($time<=0){echo "no preview receipt";}else{echo $newdate;} ?></td>
...
<table id="example5" class="table table-bordred table-striped" style="width:100%">
                        ...
                        <tbody>
							...
							$ii=0;
							while($row = mysqli_fetch_array($result)){
								$date1 = date_create_from_format('Y-m-d', $row["createdate"]);
                                $date2=date_create_from_format('Y-m-d', date("Y-m-d"));
                                $diff = (array) date_diff($date1, $date2);


                                $date=date("Y-m-d",strtotime("2018-10-17"));
                                $qry2="select GROUP_CONCAT(fl.rp_amount) as amo  from f_receipt as f inner join f_receipt_detail as fl on fl.r_id=f.id where f.s_id='".$row[0]."' and (f.cash_bill_option='Debtor' or f.cash_bill_option='Tuition Fee') and Date(r_date)>='".$date."' and  GROUP BY f.cash_bill_option";
                            ...
                            <td class="<?php echo $class; ?>"><?php if($paymonth<=0){echo $paymonth;}else{echo $paymonth;}?></td>
...
<table id="example6" class="table table-bordred table-striped" style="width:100%">
                        ...
                        <tbody>
							<?php 
                                    ...
                                    AND d.id IN (SELECT MAX(id) FROM duration GROUP BY studentid) and STR_TO_DATE(d.next_payment,'%d-%m-%Y')>date('2019-12-31')
