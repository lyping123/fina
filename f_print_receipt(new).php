...
$qry = "SELECT *,r.s_name AS old_name,r.r_date as newdate FROM f_receipt AS r
...
$num = mysqli_num_rows($sql);
                        ...
                        if($row['receipt_type'] == 2){
                            ...
                            if($row_rcp['r_no'] == ''){
                                $rno_qry = "SELECT count(fr.id) AS r_no FROM f_receipt AS fr WHERE fr.r_status = 'ACTIVE' AND fr.receipt_type = '".$row['receipt_type']."' AND fr.id BETWEEN 1 AND ".$row[0];
                                $rno_result = mysqli_query($conn, $rno_qry);
                                $rno_row = mysqli_fetch_array($rno_result);
                                $r_no = 10000 + $rno_row['r_no'];
                                $r_no = 'D'.$r_no;
