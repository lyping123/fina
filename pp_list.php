...
if(isset($_SESSION['level']) && $_SESSION['level'] == 'admin' || $_SESSION['level'] == 'superadmin'){
	$qry = "SELECT * FROM login AS l
			INNER JOIN course AS c ON c.id = l.c_id
			WHERE l.level = 'pp' AND l.l_status = 'ACTIVE' ORDER BY l.id,l.l_name,c.course DESC";
}elseif($_SESSION['dp'] == 'Department Head'){
	$qry = "SELECT * FROM login AS l
			INNER JOIN course AS c ON c.id = l.c_id
			WHERE l.level = 'pp' AND l.l_status = 'ACTIVE' AND c.id = '".$_SESSION['course']."' ORDER BY l.id,l.l_name,c.course DESC";
}
                    ...
                    <table id="mytable" class="table table-bordred table-striped" style="width:100%">
                        ...
                        <tbody>
							<?php while($row = mysqli_fetch_array($result)){
								$c_qry = "SELECT COUNT(id) AS total FROM student WHERE p_id = '".$row[0]."' AND s_status = 'ACTIVE'";
								$c_result = mysqli_query($conn,$c_qry);
								$c_rows = mysqli_fetch_array($c_result);	
							?>
                            <tr>
                            <td><a href="pp_group_list.php?id=<?=$row['0']?>"><?=$row['l_name']?></a></td>
                            <td><?=$row['course']?></td>
                            <td><?=$c_rows['total']?></td>
                            <td>
                            <!--<div class="btn-group">
