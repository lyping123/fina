...
if(isset($_POST['submit']) && $_POST['submit'] == 'save'){
	...
	if($c_rows > 0){
		...
		if(isset($_POST['t_ptpk']) && $_POST['t_ptpk'] == 't_ptpk'){
			
			$qry_rcp = "SELECT * FROM f_receipt WHERE tuition_ptpk = 'YES' AND receipt_type = '2'";
			$result_rcp = mysqli_query($conn, $qry_rcp);
			$c_row = mysqli_num_rows($result_rcp);
			$total_no = $no + $c_row;
			$new_no = 'TP'.$total_no;
		...
		$qry_save = "INSERT INTO f_receipt(debtor_ptpk,debtor,internal_exam_fee,r_no, r_date, s_name, s_ic, pay_mtd, tuition_fee, ptpk, r_status, createdate, createby, hostel_fee, receipt_type, tuition_ptpk)VALUES('".$debtor_ptpk."', '".$debtor."', '".$internal."', '".$new_no."', '".$_POST['date']."', '".mysqli_real_escape_string($conn,$_POST['name'])."', '".$_POST['ic']."', '".$pay_mtd."', '".$tuition."', '".$loan."', 'ACTIVE', '".DATE_TODAY."', '".$_SESSION['id']."', '".$hostel."', '2', '".$tuition_ptpk."')";
...
if(isset($_POST['submit']) && $_POST['submit'] == 'edit'){
	...
	$qry_save = "UPDATE f_receipt SET debtor_ptpk='".$debtor_ptpk."', debtor='".$debtor."', internal_exam_fee='".$internal."', s_name='".mysqli_real_escape_string($conn,$_POST['name'])."', s_ic='".$_POST['ic']."', pay_mtd='".$pay_mtd."', tuition_fee='".$tuition."', ptpk='".$loan."', hostel_fee='".$hostel."', tuition_ptpk='".$tuition_ptpk."'".$date." WHERE id = '".$_GET['id']."' ";
	...
	if($result_save = mysqli_query($conn, $qry_save)){
		if(isset($_POST['bankin']) && $_POST['bankin'] == 'bankin'){
			...
			if($c_rows > 0){
				...
				if($result_bankin = mysqli_query($conn, $qry_bankin)){
					echo "<script>
					window.location.href = 'f_receipt_edit2.php?action=msg_bankin_success_edit&id=$_GET[id]';
					</script>";
				}else{
					echo "<script>
					window.location.href = 'f_receipt_edit2.php?action=msg_receipt_fail_edit1&id=$_GET[id]';
					</script>";
				}
			}else{
		...
		echo "<script>
		window.location.href = 'f_receipt_edit2.php?action=msg_receipt_fail_edit&id=$_GET[id]';
		</script>";
	}
}
?>