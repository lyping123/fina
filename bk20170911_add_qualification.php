...
if(isset($_POST['submit']) && $_POST['submit'] == 'save_school'){
	        ...
	        $qry = "INSERT INTO school(s_id,name_school,location,qualification,year,status,former)VALUES('".$_GET['id']."','".$_POST['school']."','".$_POST['location']."','".$_POST['qualification']."','".$_POST['year']."','ACTIVE','".$_POST['former']."')";
...
if(isset($_POST['submit']) && $_POST['submit'] == 'edit_school'){
			...
			mysqli_set_charset($conn, 'utf8');
		  echo  $qry = "UPDATE school SET name_school='".$_POST['name']."',location='".$_POST['location']."',qualification='".$_POST['qualification']."',year='".$_POST['year']."',former='".$_POST['former']."' WHERE id = '".$_GET['id']."'";
...
if(isset($_GET['action']) && $_GET['action'] == 'edit_result'){
			...
			mysqli_set_charset($conn, 'utf8');
		    $qry = "UPDATE result SET syllabus='".$_POST['syllabus']."',result='".$_POST['result']."' WHERE id = '".$_GET['id']."'";
...
if(isset($_GET['action']) && $_GET['action'] == 'delete_school'){
	 $qry = "UPDATE school SET status = 'DELETE' WHERE id = '".$_GET['id']."'";
...
if(isset($_GET['action']) && $_GET['action'] == 'delete_result'){
	 $qry = "DELETE FROM result WHERE id = '".$_GET['id']."'";
		if($result = mysqli_query($conn, $qry)){
			echo "<script>
			window.location.href = 'qualification.php?&id=$_GET[id]&action=msg_success_delete';
			</script>";
