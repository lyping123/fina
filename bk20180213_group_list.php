...
if(isset($_SESSION['level']) && $_SESSION['level'] == 'admin' || $_SESSION['level'] == 'superadmin'){
	...
	$qry = "SELECT * FROM student_group AS s
			...
			INNER JOIN login AS l ON l.id = s.p_id
			LEFT JOIN login AS l2 ON l2.id = s.jpk_pp_id
			WHERE s.g_status = 'ACTIVE' AND c.id = '".$_SESSION['course']."' ORDER BY s.id,c.course DESC";
}else{
	$qry = "SELECT * FROM student_group AS s
			INNER JOIN course AS c ON c.id = s.c_id
			INNER JOIN login AS l ON l.id = s.p_id
			LEFT JOIN login AS l2 ON l2.id = s.jpk_pp_id
			WHERE s.g_status = 'ACTIVE' AND c.id = '".$_SESSION['course']."' AND s.p_id = '".$_SESSION['id']."' ORDER BY s.id,c.course DESC";
}
