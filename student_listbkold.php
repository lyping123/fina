                    ...
                    <table id="example1" class="table table-bordred table-striped" style="width:100%">
                        ...
                        <thead>
                          ...
                          <?php if(isset($_GET['status']) && $_GET['status'] == 'active'){?>
                            <th>Quit</th>
                          <?php }elseif(isset($_GET['status']) && $_GET['status'] == 'graduate'){?>
                            <th>Status</th>
                          <?php }elseif(isset($_GET['status']) && $_GET['status'] == 'quit'){?>
                            <th>Status</th>
                            <th>Reason Quit</th>
                            <th>Quit Date</th>
                          <?php }else{?>
                            ...
                            <tr <?php if($row['s_status'] == 'GRADUATE'){ echo 'style="background-color:  #dff0d8;"';}elseif($row['s_status'] == 'QUIT'){ echo 'style="background-color:  #dc3545;"';}else{ } ?>>
                              ...
                              <td>
                                  <?php if($row['s_status'] == 'ACTIVE'){?>
                                    <a href="#" data-title="Add" data-toggle="modal" data-target="#edit" type="button" class="btn btn-danger" id="edit_button" data-id="<?=$row[0]?>"> Quit </a>
                                  <?php }elseif($row['s_status'] == 'GRADUATE'){?>
                                    <a href="add_student.php?action=regraduate&id=<?=$row[0]?>" class="btn btn-success" onclick="return confirm('Are you sure set this student as current student?')">Reactive</a>
                                  <?php }else{ ?>
                                    <a href="add_student.php?action=requit&id=<?=$row[0]?>" class="btn btn-success" onclick="return confirm('Are you sure set this student as current student?')">Reactive</a></td>
                                <td><?php echo $row['reason_quit'];?></td>
                                <td><?php echo $row['quit_date'];?>
                                    
                                  <?php }?>
                              </td>
                                <?php if($row['s_status']=="ACTIVE"){ ?><td> <a href="add_student.php?action=graduate&id=<?=$row[0]?>" class="btn btn-danger" onclick="return confirm('Are you sure set thie student status as graduate?')">Graduate</a></td><?php } ?>
