...
if($_SESSION['dp'] == 'Department Head'){
    if(isset($_GET['status']) && $_GET['status'] == 'SUPPORT'){
        $status = "WHERE al.a_status = 'SUPPORT'";
        $order = " ORDER BY al.id DESC";
    }elseif(isset($_GET['status']) && $_GET['status'] == 'APPROVAL'){
        $status = "WHERE al.a_status = 'APPROVAL'";
        $order = " ORDER BY al.id DESC";
    }elseif(isset($_GET['status']) && $_GET['status'] == 'REJECT'){
        $status = "WHERE al.a_status = 'REJECT'";
        $order = " ORDER BY al.id DESC";
    }else{
        $status = "WHERE al.a_status = 'PENDING'";
        $order = " ORDER BY al.id DESC";
    }
...
if(isset($_GET["year"]) && $_GET["year"]!=""){
    $year=" AND YEAR(al.a_from)='$_GET[year]'";
}else{
    $year="";
}
...
if(isset($_SESSION['level']) && $_SESSION['level'] == 'admin' || $_SESSION['level'] == 'superadmin'){
	...
	$course = " AND c.id = '".$_SESSION['course']."'";
	$level = "";
	$status1 = "";
}elseif($_SESSION['dp'] == 'Department Lecturer'){
	$course = " AND c.id = '".$_SESSION['course']."'";
	$level = " AND sg.p_id = '".$_SESSION['id']."'";
	$status1 = "";
}
     $qry="SELECT * FROM apply_leave_list AS al
	  INNER JOIN student AS s ON s.id = al.s_id
	  LEFT JOIN course AS c on c.course = s .course
      LEFT JOIN (SELECT MAX(sg.g_level) AS g_level, sgl.s_id, sg.g_name, sg.p_id FROM student_group_list AS sgl
                 INNER JOIN student_group AS sg on sg.id = sgl.g_id
                 WHERE sgl.status = 'ACTIVE'".$level." GROUP BY sgl.s_id) AS sgll ON sgll.s_id = s.id
    ".$status.$course."and date(al.a_date)>date('2019-12-31') $year".$order;
...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">Student Apply Leave List
    ...
    <!--<div class="col-md-12">	
        <div class="form-group">
            ...
            <div class="col-md-12">	
                <div class="form-group">
                    <form action="studentapplyleavelist.php" method="get">
                        <div class="row">
                            <div class="col-lg-3">
                                ...
                                <div id="basic-example">
									<select name="status" class="form-control" required>
                                        ...
                                        <?php }elseif($_SESSION['dp'] == 'Department Lecturer'){?>
                        ...
                        <tbody>
                          <style>
                            ...
                            <tr>
                              ...
                              <td><?=$row['a_date']?></td>  
                              <td><?=$row['a_status']?></td>  
                              <td><a class="btn btn-primary" href="admin_leave_form.php?id=<?=$row[0]?>"> View </a></td>  
                              <td><a class="btn btn-danger" href="sl.php?action=delete&id=<?=$row[0]?>" onclick="return confirm('Are you sure delete current record?')"> Delete </a></td>      
                            </tr>
                            <?php }?>
                        </tbody>
                    </table>
