...
$qry="SELECT *, stu.course as new_course FROM student AS stu
      ...
      LEFT JOIN (SELECT MAX(sg.g_level) AS g_level, sgl.s_id, sg.g_name, sg.p_id FROM student_group_list AS sgl
                 ...
                 WHERE sgl.status = 'ACTIVE' ".$level." GROUP BY sgl.s_id) AS sgll ON sgll.s_id = stu.id
      LEFT JOIN student_leave AS sl ON sl.s_id = stu.id
	  WHERE stu.s_status = 'ACTIVE' ".$course."
	  ORDER BY stu.s_name ASC";
$result = mysqli_query($conn,$qry);
?>
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">Manage Student Leave
                        ...
                        <tbody>
                          <style>
							...
							<?php while($row = mysqli_fetch_array($result)){
                                    $history = '';
                                    $year=date("Y");
                                    $qry2 = "SELECT *, sum(a_days) AS total FROM apply_leave_list WHERE s_id = '".$row[0]."' AND a_status = 'APPROVAL' and a_date like '%$year%'  GROUP BY a_leave_type";
                                    $result2 = mysqli_query($conn, $qry2);
                                   

                                    while($row2 = mysqli_fetch_array($result2)){
                                        if($row2['a_leave_type'] == 'Leave'){
                                            $history = $row2['total'];
                                        }
                                    }


                                    $left = $row['leave'] - $history;
                            ?>
                            <tr <?php if($row['s_status'] == 'GRADUATE'){ echo 'style="background-color:  #dff0d8;"';}elseif($row['s_status'] == 'QUIT'){ echo 'style="background-color:  #dc3545;"';}else{ } ?>>
                              ...
                              <td><?=$row['g_level']?></td>
                              <td>
                                  <?php if(!empty($row['leave'])){ echo 'Total: '. $row['leave'];}else{ echo 'Total: '. '0';}?>
                                <hr>
                                  <?php echo 'Left: '. $left;?>
                              </td>    
                              <!--<td><?php if(!empty($row['medical'])){ echo 'Total: '. $row['medical'];}else{ echo 'Total: '. '0';}?></td>  
                              <td><?php if(!empty($row['emergency'])){ echo 'Total: '. $row['emergency'];}else{ echo 'Total: '. '0';}?></td>  
                              <td><?php if(!empty($row['parental'])){ echo 'Total: '. $row['parental'];}else{ echo 'Total: '. '0';}?></td>-->        
                              ...
                              <td><div class="dropdown">
                                  ...
                                  <ul class="dropdown-menu dropdown-menu-right">
                                    <li><a href="sl_form.php?id=<?=$row[0]?>">Edit</a></li>
                                  </ul>
                              </div></td>    
                            </tr>
                            <?php }?>
